...
\setbeamertemplate{footline}[frame number] % to allow frame number
...
% University of Connecticut Math REU 2020}
%  \date{\vspace{-2em}2020}
% \date{University of Connecticut Math REU 2020}
...
% Box-ball system philosophy: 
\begin{itemize}
\item At time $-\infty$, %a bunch of
...
% This is wave-like" behavior, and I believe the interest in the system comes in large part from the fact that it exhibits this behavior (and so in particular from the fact that the soliton partitions before and after are the same, even though this is very not obvious in the middle). 
\end{itemize}
  ...
  \vspace{5mm}
        ...
        % Today: Start with an initial configuration $\pi=\pi_1\pi_2\pi_3...\pi_k$, where $\pi$ is a permutation in 1-line notation.
        % \vspace{5mm}
    ...
    % Here $\pi=452361$.\\
        \RaggedRight
        \vspace{2mm}
        \boxed{\text{\textit{Step 1:}}}\phantom{---}Write the permutation on a strip of infinite boxes:\\
        % \centering
        % This configuration corresponds $t=0$.\\
        \RaggedRight
        \begin{figure}[H]
            \centering
        ...
        \boxed{\text{\textit{Step 2:}}}\phantom{---}
        % Going from smallest to largest, move each number to the next empty box to the right.\\
    ...
    \end{frame}
    \begin{frame}{Box-Ball System: Example---$\pi=452361$}
        \boxed{\text{\textit{Step 3:}}}\phantom{---}Continue moving numbers from smallest to largest to their nearest available spots until every number in the permutation has been moved. 
...
% \fi
    ...
    \begin{frame}{Box-Ball System: Example---$\pi=452361$}
        \boxed{\text{\textit{Step 5:}}}\phantom{---}After reaching steady state, 
    ...
    % When is soliton decomposition standard? When is soliton decomposition equal to the original permutation's $P$-tableau? 
    % How long does it take for a given permutation to reach its steady-state? 
...
\begin{frame}{Tableaux}
 ...
 sequences. \vspace{-0.25em}
    ...
    \begin{exampleblock}{Example. (Standard Young Tableau)}
        \begin{itemize}
        \item \raisebox{-1em}{\tiny \young(12,34,5)} is a standard  tableau. Its reading word is $53412.$ %PERM: [3, 1, 5, 4, 2]
        \item \raisebox{-1em}{\tiny \young(12,4,5,3)} is a nonstandard tableau.
        \end{itemize}
        \vspace{-3.8pt}
    \end{exampleblock}
...
\end{itemize} 
    ...
    % Let $P$ be the empty tableau. Given a permutation $\pi = \pi_1\cdots\pi_n,$ the \textit{Robinson-Schensted-Knuth (RSK) insertion algorithm} is a method of insertion of each element of $\pi$ into $P.$ The RSK insertion algorithm ensures that the result---after all elements of $\pi$ have been inserted---will be a standard Young tableau.
        ...
        \begin{block}{REU Questions.}
    % When is a soliton decomposition standard?
    % For what permutations $\pi$ do we have soliton decomposition $SD(\pi)$ equal the insertion tableau $P(\pi)$?
    
     For what permutations $\pi$ do we have  $SD(\pi)=P(\pi)$?
    
    % When is a soliton decomposition equal to the  permutation's $P$-tableau? 
    \end{block}
...
%     \begin{itemize}
%         \item 
% It is sufficient to show $(3)\implies(1)$ or to show $(3) \implies (2)$.
%     \end{itemize}
\end{frame}
...
\begin{frame}{Soliton Decomposition and Knuth moves}
    \begin{itemize}
            % \item Yet another way of attacking this problem is from the perspective of Knuth moves
            % \footnote[frame]{The theorem on this slide is due in part to a result in \cite{LLPS19}.}
...
\begin{frame}[noframenumbering]%[focus]
\Huge
\Centering
        Thank You! 
        % \\
